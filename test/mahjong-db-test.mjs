...
try {
  ...
  const backend = run(f1, `db.savePlayer('u1', { name: '阿强', total: 42, pots: 3 });
  ...
  ok(out1.sb.u1 && out1.sb.u1.total === 42 && out1.sb.u1.pots === 3 && out1.sb.u1.name === '阿强', 'leaderboard player survived the restart');
  ...
  run(f2, `db.savePlayer('u1', { name:'阿强', total:10, pots:1 });`);
  run(f2, `db.savePlayer('u1', { name:'阿强', total:25, pots:2 });`); // server keeps a running total in memory; here we just verify the latest sticks
  const out2 = JSON.parse(run(f2, `process.stdout.write(JSON.stringify(db.loadScoreBook()));`));
  ok(out2.u1 && out2.u1.total === 25 && out2.u1.pots === 2, 'updated lifetime total persisted');
  cleanup(f2);
  ...
  ok(out3.sb.uLegacy && out3.sb.uLegacy.total === 77 && out3.sb.uLegacy.pots === 5, 'legacy leaderboard recovered into the DB');
  ...
  ok(out3b.uLegacy && out3b.uLegacy.total === 77, 'migrated data persists on the next open');
