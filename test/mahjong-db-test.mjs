...
try {
  ...
  const backend = run(f1, `db.savePlayer('tianjin', 'u1', { name: '阿强', total: 42, matches: 3 });
    db.savePlayer('guobiao', 'u1', { name: '阿强', total: 5, matches: 1 });
  ...
  ok(out1.sb.tianjin && out1.sb.tianjin.u1 && out1.sb.tianjin.u1.total === 42 && out1.sb.tianjin.u1.matches === 3 && out1.sb.tianjin.u1.name === '阿强', 'leaderboard player survived the restart');
  ...
  ok(out1.t[0] && out1.t[0].match && out1.t[0].match.scores[3] === 4 && out1.t[0].match.dealer === 2, 'match snapshot survived the restart');
  ...
  run(f2, `db.savePlayer('tianjin', 'u1', { name:'阿强', total:10, matches:1 });`);
  run(f2, `db.savePlayer('tianjin', 'u1', { name:'阿强', total:25, matches:2 });`); // server keeps a running total in memory; here we just verify the latest sticks
  const out2 = JSON.parse(run(f2, `process.stdout.write(JSON.stringify(db.loadScoreBook()));`));
  ok(out2.tianjin && out2.tianjin.u1 && out2.tianjin.u1.total === 25 && out2.tianjin.u1.matches === 2, 'updated lifetime total persisted');
  cleanup(f2);
  ...
  // the legacy on-disk JSON uses the old `pots`/`pot` keys — the migration must still recover them
  ...
  ok(out3.sb.tianjin && out3.sb.tianjin.uLegacy && out3.sb.tianjin.uLegacy.total === 77 && out3.sb.tianjin.uLegacy.matches === 5, 'legacy leaderboard recovered onto the 天津 board');
  ok(out3.t[0] && out3.t[0].match && out3.t[0].match.scores[0] === 8 && out3.t[0].match.prevailingWind === 2, 'legacy match snapshot recovered into the DB');
